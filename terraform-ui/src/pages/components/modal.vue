        ...
        <div class="modal-content c-dark">
          ...
          <div class="modal-body">
            <form class="">
              <div v-for="(item, index) in modelConfig.config" class="params-each" :key="index">
                <label
                  class="col-md-2 label-name"
                  v-if="item.type != 'slot' && isHide(item.hide)"
                  :class="item.type === 'select' ? 'lable-name-select' : ''"
                >
                  {{ $t(item.label) }}:</label
                >
                <input
                  v-if="item.type === 'text' && isHide(item.hide)"
                ...
                />
                <input
                  v-if="item.type === 'number' && isHide(item.hide)"
                ...
                />
                <input
                  v-if="item.type === 'password' && isHide(item.hide)"
                  v-model="modelConfig.addRow[item.value]"
                ...
                />
                <input
                  v-if="item.type === 'checkbox' && isHide(item.hide)"
                  v-model="modelConfig.addRow[item.value]"
                  autocomplete="off"
                  :type="item.type"
                  :disabled="modelConfig.isAdd ? false : item.disabled"
                  class="checkbox"
                />
                <i-switch v-if="item.type === 'switch' && isHide(item.hide)" v-model="modelConfig.addRow[item.value]" />
                <textarea
                ...
                <label class="required-tip  isRequired_s" v-if="isRequired_s(item.v_validate) && isHide(item.hide)"
                  >*</label
                >
                ...
                <Poptip
                  v-if="item.tips && isHide(item.hide) && item.type !== 'switch'"
                  word-wrap
                  content=""
                  trigger="hover"
                  placement="bottom"
          ...
          <template v-if="!modelConfig.noBtn">
            <div class="model-footer-f c-dark" v-if="!modelConfig.modalFooter">
              ...
              <button v-if="!modelConfig.saveFunc" @click="save(modelConfig.isAdd)" type="button" class="btn-confirm-f">
                {{ $t('button.save') }}
              </button>
              <button
                v-if="modelConfig.saveFunc"
                @click="customFunc(modelConfig.saveFunc)"
                type="button"
                class="btn-confirm-f"
              >
                {{ $t('button.save') }}
...
export default {
  ...
  methods: {
    ...
    isHide (val) {
      ...
      if (res === String(val).toUpperCase() || val === true) {
        // val,适用于配置项显示隐藏不依据新建/编辑状态，依赖其他条件，配置boolean值：false显示，true不显示
        return false
      }
      return true
    }
