...
func SaveOperationLog(param *models.SysLogTable) {
	_, err := x.Exec("INSERT INTO sys_log(log_cat,operator,operation,content,request_url,client_host,created_date) value (?,?,?,?,?,?,?)",
		param.LogCat, param.Operator, param.Operation, param.Content, param.RequestUrl, param.ClientHost, time.Now().Format(models.DateTimeFormat))
	if err != nil {
