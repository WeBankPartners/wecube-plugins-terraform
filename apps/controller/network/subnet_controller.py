...
class ResBase(object):
    ...
    def allow_key(cls, data):
        validation.allowed_key(data, ["id", "provider", "secret", "region", "zone",
                                      "name", "vpc_id", "cidr", "extend_info"])

    @classmethod
    def not_null(cls, data):
        validation.not_allowed_null(data=data,
                                    keys=["region", "provider", "name", "vpc_id", "cidr"]
                                    )
    ...
    def create(cls, resource, data, **kwargs):
        ...
        create_data = {"name": name, "cidr": cidr, "vpc_id": vpc_id}
...
class SubnetAddController(BackendAddController):
    allow_methods = ("POST",)
    resource = SubnetBackendApi()


class SubnetDeleteController(BackendDeleteController):
    name = "Subnet"
...
class SubnetSourceController(BackendSourceController):
    name = "Subnet"
    resource_describe = "Subnet"
    allow_methods = ("POST",)
    resource = SubnetBackendApi()
