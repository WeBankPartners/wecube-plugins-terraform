...
from apps.api.network.ccn_bandwidth import CCNBandwidthBackendApi
...
class ResBase(object):
    ...
    def allow_key(cls, data):
        validation.allowed_key(data, ["id", "provider", "secret", "region", "zone",
                                      "name", "ccn_id", "from_region",
                                      "dest_region", "bandwidth", "extend_info"])

    @classmethod
    def not_null(cls, data):
        validation.not_allowed_null(data=data,
                                    keys=["region", "provider", "ccn_id"]
                                    )
    ...
    def create(cls, resource, data, **kwargs):
        ...
        create_data = {"name": name, "ccn_id": ccn_id,
                       "bandwidth": bandwidth,
...
class CCNBandwidthAddController(BackendAddController):
    allow_methods = ("POST",)
    resource = CCNBandwidthBackendApi()


class CCNBandwidthDeleteController(BackendDeleteController):
    name = "CCNBandwidth"
...
class CCNBandSourceController(BackendSourceController):
    name = "CCNBandwidth"
    resource_describe = "CCNBandwidth"
    allow_methods = ("POST",)
    resource = CCNBandwidthBackendApi()
