...
from apps.background.models.dbserver import ProviderSecretManager


class ProviderSecretObject(object):
    def __init__(self):
        self.resource = ProviderSecretManager()
    ...
    def create(self, create_data):
        if create_data.get("secret_info"):
            _key = create_data.get("secret_info")
            if not _key.startswith("{cipher_a}"):
                create_data["secret_info"] = "{cipher_a}" + encrypt_str(create_data.get("secret_info"))
    ...
    def update(self, rid, update_data, where_data=None):
        if update_data.get("secret_info"):
            _key = update_data.get("secret_info")
            if not _key.startswith("{cipher_a}"):
                update_data["secret_info"] = "{cipher_a}" + encrypt_str(update_data.get("secret_info"))
    ...
    def secret_name_object(self, name, provider):
        data = self.query_one(where_data={"name": name, "provider": provider})
        if not data:
            raise local_exceptions.ResourceValidateError("secret", "secret %s 未注册" % name)
        return data

    def name_object(self, name, provider):
        return self.query_one(where_data={"name": name, "provider": provider})
