...
class ObjectStorageObject(_StorageBaseObject):
    def __init__(self):
        super(ObjectStorageObject, self).__init__()
        self.resource = ObjectStorageManager()

    def resource_id(self, rid):
        data = self.show(rid)
        if not data:
            raise local_exceptions.ValueValidateError("object storage", "object storage %s 不存在" % rid)
        return data["resource_id"]


class BucketObjectObject(_StorageBaseObject):
    def __init__(self):
        super(BucketObjectObject, self).__init__()
        self.resource = BucketObjectManager()
