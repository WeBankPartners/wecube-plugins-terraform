...
class ENIObject(_ENIBaseObject):
    def __init__(self):
        super(ENIObject, self).__init__()
        self.resource = NetworkInterfaceManager()

    def disk_resource_id(self, rid):
        data = self.show(rid)
        if not data:
            raise local_exceptions.ValueValidateError("ENI", "network interface id %s 不存在" % rid)
        return data["resource_id"]


class ENIAttachObject(_ENIBaseObject):
    def __init__(self):
        super(ENIAttachObject, self).__init__()
        self.resource = NetworkInterfaceAttachManager()
